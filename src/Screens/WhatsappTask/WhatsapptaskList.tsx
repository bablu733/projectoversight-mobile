...
const WhatsappTaskListScreen: React.FC = () => {
  ...
  const loadConnectionList = async () => {
    ...
    setLoading(true);
    try {
      const response = await axios.privateAxios.get<EmployeeDailyTask[]>("/app/EmployeeTask/GetWhatsapptaskListByTaskId?Status=" + Status.InProgress + "&employeeId=" + employeeId + "&WeekEndingDate=" + formattedWeekEndingDate);
      setLoading(false);
      // Filter items with "WorkedOn Date" as today
      const today = new Date();
      const filteredList = response.data.filter((item) => {
        // Format the "WorkedOn" date to match the format of the today variable
        ...
        // Compare the formatted "WorkedOn" date with the today variable
