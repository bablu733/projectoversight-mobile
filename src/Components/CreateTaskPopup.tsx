...
interface TaskDetailsPopupProps {
  ...
  isVisible: boolean;
  onClose: () => void;
  onSubmit: (estStartDate: string, estTime: string, comments: string, estEndDate: string, taskType: string, classification: string, weekEndDate: string,TaskDescription:string,selectedImages:File) => void;
}
...
const TaskDetailsPopup: React.FC<TaskDetailsPopupProps> = ({
  ...
  const handleSubmit = () => {
    ...
    Vibration.vibrate();
      ...
      onSubmit(estStartDate, estTime, comments, estEndDate, TaskTypeValue, ClassificationValue, weekEndDate,TaskDescription,selectedImages);
  }; 
  useEffect(() => {
    loadConnectionList();
  }, []);
